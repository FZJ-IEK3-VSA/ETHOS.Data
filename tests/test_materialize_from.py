...
def _resource(path: str, payload: bytes) -> Resource:
    return Resource('example', path.replace('/', '-'), path, len(payload),
...
def test_the_copy_is_still_checked_against_the_manifest(workspace, tmp_path):
    ...
    wrong = _write(tmp_path / 'wrong', {'a.txt': b'not it', 'sub/b.txt': CONTENT['sub/b.txt']})
