...
def _no_network(*args, **kwargs):
    ...
    """
    pytest.fail("the public catalogue was contacted; the pinned one should have been read")
...
def world(tmp_path, monkeypatch):
    ...
    for variable in ("ETHOS_DATA_CATALOG", "ETHOS_STAGING_DIR", "ETHOS_RESTRICTED_DIR",
                     "ETHOS_SKIP_UNAVAILABLE", "ETHOS_PUBLICATION_URL"):
    ...
    entries.append({"name": "licensed", "path": "datasets/licensed/datapackage.json",
                    "ethos:access": "restricted", "ethos:license_status": "resolved"})
    licensed = catalogue / "datasets" / "licensed" / "datapackage.json"
    licensed.parent.mkdir(parents=True)
    licensed.write_text(json.dumps({"name": "licensed", "resources": [
        {"name": "secret.tif", "path": "secret.tif", "bytes": 3, "hash": "sha256:00"}]}))
...
def define(world):
    ...
    def write(*bodies: str, catalog: Path = index, name: str = "collections.yaml") -> Path:
        ...
        collections = "".join(textwrap.dedent(body).strip("\n") + "\n" for body in bodies)
        path = tmp_path / name
        path.write_text(
...
class TestVariants:
    def test_the_test_flag_selects_the_test_variant_and_full_is_the_default(self, define):
        """Full is the default: a forgotten flag must download visibly, never
        run a real calculation on fixtures in silence."""
        file = define(ONSHORE)
        assert sorted(ethos_data.fetch("onshore_wind", file, progressbar=False, test=True)) == TEST_KEYS
        assert sorted(ethos_data.fetch("onshore_wind", file, progressbar=False)) == FULL_KEYS
        assert sorted(ethos_data.fetch("onshore_wind", file, progressbar=False, test=False)) == FULL_KEYS
    ...
    def test_a_plain_collection_is_the_same_either_way(self, define):
        ...
        assert dict(with_flag) == dict(without) == {"landcover/clc.tif": with_flag["landcover/clc.tif"]}
        assert with_flag.named == without.named == {"clc": with_flag["landcover/clc.tif"]}
    ...
    def test_the_collections_object_reports_its_variants(self, define):
        ...
        assert loaded.definition("onshore_wind", test=True)["paths"]["era5"] == "reskit-test-data/era5"
    ...
    def test_the_module_constants_name_exactly_two_variants(self):
        ...
        assert selection.VARIANTS == (selection.VARIANT_TEST, selection.VARIANT_FULL) == ("test", "full")
    ...
    def test_the_test_flag_propagates_through_extends(self, define):
        ...
        assert sorted(small) == ["landcover/clc.tif",
    ...
    def test_variants_must_offer_the_same_handles(self, define):
        ...
        with pytest.raises(ethos_data.CollectionError, match="only in test: gwa_50m; only in full: gwa_200m"):
            ethos_data.resolve("lopsided", file)
        # Checked whichever variant is asked for: the mistake is in the file, not the call.
        with pytest.raises(ethos_data.CollectionError, match="must name the same paths"):
    ...
    def test_a_plain_child_of_lopsided_variants_is_refused_too(self, define, download_spy):
        ...
        assert [r.key for r in ethos_data.resolve("onshore_wind", file, test=True)] == TEST_KEYS

    def test_a_variant_that_cannot_be_resolved_is_reported_as_a_failed_comparison(self, define):
            ...
            mixed:
              ...
              full:
                include:
                  - dataset: landcover
            """)
        with pytest.raises(ethos_data.CollectionError, match="include at the top level"):
            ethos_data.resolve("mixed", file)

    def test_asking_for_a_variant_that_is_not_defined_says_how_to_get_the_other(self, define):
            ...
            full_only:
              full:
                include:
                  - dataset: landcover
            """)
        with pytest.raises(ethos_data.CollectionError, match=r"test=True \(or --test\)"):
    ...
    def test_an_unknown_collection_is_an_unknown_collection_error(self, define):
        with pytest.raises(ethos_data.UnknownCollection, match="unknown collection 'nope'"):
...
class TestNamedPaths:
    ...
    def test_a_folder_handle_is_the_directory_holding_the_selected_files(self, world, define):
    ...
    def test_a_missing_handle_names_the_collection_and_lists_what_it_defines(self, define):
        """The handles are the maintainer's vocabulary; a typo should say so, not
        fail three frames later inside a raster reader."""
        inputs = ethos_data.paths("onshore_wind", define(ONSHORE), progressbar=False, test=True)
    ...
    def test_a_collection_without_paths_has_empty_named_and_refuses_paths(self, define):
        ...
        assert files.named == {}
        assert sorted(files) == ["landcover/clc.tif", "landcover/sites.dbf", "landcover/sites.shp"]
...
class TestInheritedHandles:
    def test_handles_are_inherited_through_extends(self, world, define):
        ...
        assert inputs["speed"] == cache / "global-wind-atlas-v3/gwa3_250_wind-speed_100m.tif"
    ...
    def test_two_parents_disagreeing_is_an_error_the_child_can_settle(self, world, define, download_spy):
        ...
        file = define(PARENTS, """
            ...
            settled:
              ...
              paths:
                raster: landcover/clc.tif
            """)
        with pytest.raises(ethos_data.CollectionError, match="inherits paths.raster from both 'a'") as caught:
            ethos_data.fetch("conflicted", file, progressbar=False)
        assert "define paths.raster in 'conflicted'" in str(caught.value)
        assert download_spy == [], "the definition was rejected, so nothing may be fetched"
        assert ethos_data.paths("settled", file, progressbar=False)["raster"] == cache / "landcover/clc.tif"
    ...
    def test_two_parents_agreeing_is_not_a_conflict(self, define):
        file = define(PARENTS, """
            ...
            child:
              extends: [b, twin]
            """)
        assert ethos_data.load_collections(file).named_keys("child")["speed"].endswith("100m.tif")
...
class TestValidationBeforeDownload:
    ...
    def test_a_file_handle_must_be_included_by_the_collection(self, define, download_spy):
        file = define(FORGOT)
        with pytest.raises(ethos_data.CollectionError) as caught:
            ethos_data.fetch("forgot", file, progressbar=False)
        message = str(caught.value)
        assert "paths.clc names the file 'landcover/clc.tif', which the collection does not include" in message
        assert "add it under 'include:' (dataset: landcover, files: ['clc.tif'])" in message
        assert download_spy == []

    def test_a_folder_handle_needs_at_least_one_selected_file_beneath_it(self, define, download_spy):
        file = define("""
            hollow:
              include:
                - dataset: era5
    ...
    @pytest.mark.parametrize("key", ["nowhere/file.tif", "era5/2099", "landcover/clc.tiff",
                                     "reskit-test-data/nobody/x.nc"])
    def test_a_handle_naming_something_the_catalogue_lacks(self, define, download_spy, key):
        file = define(f"""
            lost:
              include:
                - dataset: landcover
              paths:
                thing: {key}
            """)
        with pytest.raises(ethos_data.CollectionError, match="not in the catalogue") as caught:
            ethos_data.fetch("lost", file, progressbar=False)
        assert f"paths.thing names {key!r}" in str(caught.value)
        assert download_spy == []

    def test_validation_passes_before_the_download_and_then_downloads_once(self, define, download_spy):
    ...
    def test_unreachable_data_is_an_access_error_unless_the_caller_said_to_skip_it(
        ...
        message = str(caught.value)
        assert message.startswith("dataset 'licensed' is restricted and is never downloaded.")
        assert "--skip-unavailable" in message
        with pytest.raises(ethos_data.AccessError):
            ethos_data.fetch("with_licensed", file, progressbar=False, skip_unavailable=False)
        ...
        with pytest.warns(UserWarning):
            assert ethos_data.fetch("with_licensed", file, progressbar=False).named.omitted == ["secret"]

    def test_under_skip_unavailable_an_unreachable_handle_is_left_out_and_named(self, world, define):
        ...
        with pytest.warns(UserWarning) as record:
            files = ethos_data.fetch("with_licensed", file, progressbar=False, skip_unavailable=True)
        messages = [str(warning.message) for warning in record]
        assert any(m.startswith("1 file(s) from licensed are not available on this machine") for m in messages)
        assert (
        ...
        with pytest.warns(UserWarning) as record:
            inputs = ethos_data.paths("with_licensed", file, progressbar=False, skip_unavailable=True)
        ...
        with pytest.raises(KeyError) as caught:
            inputs["secret"]
        message = caught.value.args[0]
        assert "named path 'secret' in collection 'with_licensed' is not available on this machine" in message
    ...
    def test_paths_is_empty_not_an_error_when_every_handle_was_left_out(self, define):
        ...
        file = define("""
            ...
            """)
        with pytest.warns(UserWarning) as record:
            inputs = ethos_data.paths("only_secret", file, progressbar=False, skip_unavailable=True)
        assert any("named path(s) secret" in str(w.message) for w in record)
        assert inputs == {}
        assert inputs.omitted == ["secret"]
...
def incomplete(world):
    ...
    document["datasets"] += [
        {"name": "ghost", "path": "datasets/ghost/datapackage.json",
         "ethos:license_status": "resolved"},
        {"name": "sharded", "path": "datasets/sharded/datapackage.json",
         "ethos:license_status": "resolved"},
    ]
...
class TestIncompleteCatalog:
    def test_a_missing_descriptor_is_diagnosed_as_an_incomplete_catalogue(self, incomplete):
    ...
    def test_a_collection_selecting_from_a_ghost_dataset_raises_it(self, define, incomplete, download_spy):
        file = define("""
            haunted:
              include:
                - dataset: ghost
...
class TestCommandLine:
    def test_list_prints_one_row_per_variant_and_flags_what_it_cannot_resolve(self, define, incomplete, capsys):
        ...
        out = capsys.readouterr().out
        assert re.search(r"^  landcover\s+1 files\s+\d+ B\s+Land cover$", out, re.MULTILINE)
        assert re.search(r"^  onshore_wind \[test\]\s+6 files\s+\d+ B\s+Data for onshore wind workflows$", out, re.MULTILINE)
        # The title is printed once per collection, on its first row.
        assert re.search(r"^  onshore_wind \[full\]\s+7 files\s+\d+ B\s*$", out, re.MULTILINE)
        assert re.search(r"^  lopsided \[test\]\s+\[unresolvable\]\s+collection 'lopsided'", out, re.MULTILINE)
        assert re.search(r"^  lopsided \[full\]\s+\[unresolvable\]", out, re.MULTILINE)
        assert re.search(r"^  haunted\s+\[unresolvable\]\s+dataset 'ghost' is listed", out, re.MULTILINE)
    ...
    def test_info_labels_the_variant_and_shows_the_named_paths(self, define, capsys):
        ...
        assert re.search(r"^  gwa_100m\s+->\s+reskit-test-data/global-wind-atlas/gwa100-like.tif$", out, re.MULTILINE)
    ...
    def test_fetch_with_the_test_flag_labels_its_message(self, define, capsys):
        ...
        assert capsys.readouterr().out.startswith("onshore_wind [test]: all 6 available files already present")
        assert main(["-c", str(file), "fetch", "onshore_wind"]) == 0
        assert capsys.readouterr().out.startswith("onshore_wind [full]: all 7 available files already present")

    def test_paths_prints_tab_separated_handle_and_path_lines(self, world, define, capsys):
        """Tab-separated so a shell can read it back: ``while IFS=$'\\t' read handle path``."""
        _, cache, _ = world
        assert main(["-c", str(define(ONSHORE)), "paths", "onshore_wind", "--test"]) == 0
        ...
        assert rows == {
            ...
            "gwa_100m": str(cache / "reskit-test-data/global-wind-atlas/gwa100-like.tif"),
            "gwa_50m": str(cache / "reskit-test-data/global-wind-atlas/gwa50-like.tif"),
            "gwa_200m": str(cache / "reskit-test-data/global-wind-atlas/gwa200-like.tif"),
        }

    def test_paths_on_a_collection_without_paths_is_an_error_not_a_traceback(self, define, capsys):
        file = define("""
            bare:
              include:
                - dataset: landcover
        ...
        captured = capsys.readouterr()
        assert captured.out == ""
        assert captured.err.startswith("error: collection 'bare' declares no named paths")
    ...
    def test_ls_lists_the_catalogue_without_fetching(self, world, download_spy, capsys):
        ...
        assert re.search(r"^  reskit-test-data/era5/100m_u_component_of_wind.nc\s+1 B$", out, re.MULTILINE)
        assert re.search(r"^  reskit-test-data/era5/forecast_surface_roughness.nc\s+3 B$", out, re.MULTILINE)
    ...
    def test_an_unknown_collection_is_a_message_not_a_traceback(self, define, capsys, command):
    ...
    def test_an_incomplete_catalogue_is_a_message_not_a_traceback(self, define, incomplete, capsys):
        file = define("""
            haunted:
              include:
                - dataset: ghost
    ...
    def test_a_misdefined_collection_is_a_message_not_a_traceback(self, define, capsys):
        ...
        assert capsys.readouterr().err.startswith("error: collection 'forgot': paths.clc names the file")

    @pytest.mark.parametrize("command", ["info", "plan", "fetch"])
    def test_a_bad_handle_stops_every_command_before_it_reports_or_downloads(
        self, define, download_spy, capsys, command
    ):
    ...
    def test_list_flags_a_bad_handle_and_a_collection_that_is_not_a_mapping(self, define, capsys):
        ...
        file = define(ONSHORE, FORGOT, """
            broken: null
            alsobroken: [a, b]
            """)
        assert main(["-c", str(file), "list"]) == 1
        out = capsys.readouterr().out
        assert re.search(r"^  forgot\s+\[unresolvable\]\s+collection 'forgot': paths\.clc names the file",
                         out, re.MULTILINE)
        assert re.search(r"^  broken\s+\[unresolvable\]\s+collection 'broken' must be a mapping .*got NoneType$",
                         out, re.MULTILINE)
        assert re.search(r"^  alsobroken\s+\[unresolvable\]\s+collection 'alsobroken' must be a mapping .*got list$",
                         out, re.MULTILINE)
    ...
    def test_paths_under_skip_unavailable_prints_only_the_handles_this_machine_can_honour(
        ...
        file = define(LICENSED)
        with pytest.warns(UserWarning) as record:
            assert main(["--skip-unavailable", "-c", str(file), "paths", "with_licensed"]) == 0
        assert capsys.readouterr().out.splitlines() == [f"clc\t{cache / 'landcover/clc.tif'}"]
        assert any("named path(s) secret are not available on this machine" in str(w.message)
                   for w in record)
        # Without the flag the command stops, as the API does, before any transfer.
        assert main(["-c", str(file), "paths", "with_licensed"]) == 2
        captured = capsys.readouterr()
        assert captured.out == ""
        assert captured.err.startswith("error: dataset 'licensed' is restricted and is never downloaded.")
    ...
    def test_verify_all_skips_what_it_cannot_resolve_and_says_so_in_its_exit_status(
        ...
        out = capsys.readouterr().out
        assert re.search(r"^skipped spooky \[full\]: dataset 'ghost' is listed in the catalogue index",
                         out, re.MULTILINE)
        assert re.search(r"^skipped lopsided \[test\]: collection 'lopsided': its test and full variants "
                         r"must name the same paths", out, re.MULTILINE)
        assert re.search(r"^skipped lopsided \[full\]: collection 'lopsided'", out, re.MULTILINE)
        ...
        assert main(["-c", str(define(ONSHORE, name="fine.yaml")), "verify", "--all"]) == 0
    ...
    def test_the_test_flag_may_come_before_the_subcommand(self, define, capsys):
        ...
        file = define(ONSHORE)
        outputs = []
        for argv in (["-c", str(file), "info", "onshore_wind", "--test"],
    ...
    def test_ls_and_path_read_the_catalogue_the_collections_file_pins(
        ...
        assert capsys.readouterr().out.startswith("reskit-test-data/era5: 3 files, 5 B\n")
        ...
        assert main(["--catalog", str(other_catalog), "-c", str(file), "ls", "reskit-test-data/era5"]) == 2
        assert capsys.readouterr().err.startswith("error: unknown dataset 'reskit-test-data'")
...
class TestCatalogUnavailable:
    ...
    def test_a_missing_local_index_names_the_path_and_says_how_to_point_elsewhere(self, world):
        ...
        for advice in ("--catalog", "$ETHOS_DATA_CATALOG", "ethos-data config set-catalog"):
            assert advice in message
        assert not issubclass(ethos_data.IncompleteCatalog, ethos_data.CatalogUnavailable)

    def test_an_http_404_and_an_unreachable_host_are_the_same_diagnosis(self, world, monkeypatch):
        ...
        message = str(caught.value)
        assert message.startswith(f"cannot read the catalogue index at {url}: HTTP 404 Not Found")
    ...
    def test_the_cli_prints_a_message_and_exits_two(self, world, define, capsys):
        ...
        for argv in (["-c", str(file), "info", "onshore_wind"],
                     ["-c", str(file), "list"],
                     ["-c", str(file), "ls", "era5"]):
            assert main(argv) == 2
            captured = capsys.readouterr()
...
class TestCatalogPin:
    ...
    def test_a_relative_pin_belongs_to_the_file_not_the_working_directory(self, tmp_path, monkeypatch):
        ...
        file.write_text("catalog: ../catalogue/datacatalog.json\ncollections: {}\n", encoding="utf-8")
        monkeypatch.chdir(tmp_path)
        expected = str((tmp_path / "project" / "catalogue" / "datacatalog.json").resolve())
    ...
    def test_no_pin_is_none_and_a_parsed_document_may_be_handed_in(self, tmp_path):
        ...
        assert selection.catalog_pin(tmp_path / "never-written.yaml", {"catalog": "index.json"}) == str(
            (tmp_path / "index.json").resolve()
        )
    ...
    def test_a_legacy_ref_suffix_is_stripped(self, tmp_path):
        ...
        file = tmp_path / "collections.yaml"
        assert selection.catalog_pin(file, {"catalog": "catalogue/datacatalog.json@v1.2"}) == str(
            (tmp_path / "catalogue" / "datacatalog.json").resolve()
        )
        assert (selection.catalog_pin(file, {"catalog": "https://example.invalid/cat/datacatalog.json@v2"})
    ...
    def test_load_collections_follows_the_pin_from_any_working_directory(self, world, define, monkeypatch):
        ...
        assert [r.key for r in ethos_data.resolve("landcover", file)] == ["landcover/clc.tif"]
        ...
        file = define("""
            only_licensed:
              include:
                - dataset: licensed
        """)
        assert main(["--skip-unavailable", "-c", str(file), "fetch", "only_licensed"]) == 0
        out = capsys.readouterr().out
