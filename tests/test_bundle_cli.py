...
def test_cli_export_verify_development_override(tmp_path, monkeypatch, capsys):
    ...
    package = {
        'name': 'lesson', 'ethos:access': 'public', 'ethos:license_status': 'resolved',
        'resources': [{'name': 'value', 'path': 'value.txt', 'bytes': 1,
                       'hash': 'sha256:' + hashlib.sha256(b'3').hexdigest()}],
    }
    (tmp_path / 'datapackage.json').write_text(json.dumps(package))
    (tmp_path / 'datacatalog.json').write_text(json.dumps({
        'ethos:publication_url': 'https://example.invalid/data',
        'datasets': [{'name': 'lesson', 'path': 'datapackage.json', 'ethos:access': 'public'}],
    }))
    collections = tmp_path / 'collections.yaml'
    collections.write_text('catalog: datacatalog.json\ncollections:\n  small:\n    include:\n      - dataset: lesson\n')
    target = tmp_path / 'bundle'
    assert main(['-c', str(collections), 'bundle', 'export', str(target), 'small',
                 '--source-root', f'lesson={source}', '--source-revision', 'test']) == 0
