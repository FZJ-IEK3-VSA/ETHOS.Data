...
def test_overlay_does_not_mutate_canonical_catalogue(workspace):
    ...
    canonical = ethos_data.load_collections(
        collections, catalog=catalog, include_staging=False
    )
...
def test_broken_staging_link_fails_python_and_cli(workspace, capsys):
    ...
    (root / "staging" / "broken").symlink_to(
    ...
    for call in (
        ...
        lambda: ethos_data.catalog(str(root / "datacatalog.json")).path(
            "broken/input.txt"
        ),
    ):
