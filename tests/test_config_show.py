...
def public(monkeypatch, tmp_path) -> Path:
    ...
    for variable in (config.RESTRICTED_ENV_VAR, config.STAGING_ENV_VAR, config.CATALOG_ENV_VAR,
                     config.SKIP_UNAVAILABLE_ENV_VAR, "ETHOS_PUBLICATION_URL"):
...
class TestTheListingIsOneDirectoryRead:
    def test_a_dataset_without_top_level_files_is_not_walked(self, public, monkeypatch, capsys):
        ...
        assert "public cache holds 0 link(s) and 2 director(ies) at the top level:" in out
        assert "  era5" in out and "  flat" in out
        assert era5 not in listed and (era5 / "2000") not in listed

    def test_the_catalogue_position_comes_before_the_cache_contents(self, public, capsys):
        public.mkdir()
        (public / "one").mkdir()
        out = show(capsys)
    ...
    def test_links_are_listed_with_their_targets(self, public, tmp_path, capsys):
        ...
        out = show(capsys)
        assert "public cache holds 1 link(s) and 0 director(ies) at the top level:" in out
...
class TestAnUnreachableCacheSaysWhy:
    ...
    def test_a_missing_restricted_cache_is_flagged(self, public, monkeypatch, tmp_path, capsys):
    ...
    def test_an_error_from_the_operating_system_is_repeated(self, public, monkeypatch, capsys):
        ...
        out = show(capsys)
        assert ("[NOT REACHABLE -- cannot be reached "
                "(The specified network name is no longer available)]") in out
    ...
    def test_a_per_dataset_root_that_is_gone(self, public, monkeypatch, tmp_path, capsys):
        gone = tmp_path / "gone"
        monkeypatch.setattr(config, "load_config", lambda: (
            {"dataset_roots": {"era5": str(gone)}},
            {"dataset_roots": "user config x", "dataset_roots.era5": "user config x"},
        ))
        out = show(capsys)
    ...
    def test_a_disconnected_drive_is_named(self):
        ...
        free = next(f"{letter}:" for letter in reversed(string.ascii_uppercase)
                    if not Path(f"{letter}:/").exists())
        assert _unreachable(Path(f"{free}/shared_data/public")) == f"drive {free} is not connected"
