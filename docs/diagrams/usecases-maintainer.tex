...
% The command here is top-level rather than a `catalog` subcommand, and that
% is why this oval hangs off the actor instead of joining the row above it:
% filling a cache runs on the machine that already holds the bytes, publishes
% nothing, and so has no position in the ordering the rest of this diagram
% exists to show. It shares the maintainer with them, and nothing else.
\node[writecase, below=15mm of describe] (link) {Build the shared cache};
% The flag is spelled -{}-all because a bare `--` is still read as an en-dash
% ligature here, and the first render of this node silently produced `link
% -all`: a command a reader could copy and be refused by.
\node[cmd, below=2.5mm of link] (link-c) {link -{}-all};
...
% instead sent a long line straight through the shared-cache oval, which read as
...
\end{scope}
% Labelled for the tool rather than the `catalog` group: the shared-cache oval
% above is `ethos-data link`, so a boundary called `ethos-data catalog` would
% now be claiming a command that deliberately lives outside that group.
\node[boundarylabel, anchor=north west] at (sys.north west) {ethos-data};
