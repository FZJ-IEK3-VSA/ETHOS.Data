...
`ethos-data verify` is the one place where the promise "these bytes are the ones
...
ethos-data verify --all                   # every collection in the file
ethos-data verify --all --deep -q         # only report problems
```
...
```bash
ethos-data verify --all --deep --repair --dry-run   # say what would be re-fetched
ethos-data verify --all --deep --repair            # do it
```
...
ethos-data materialize --all                      # every entry that is a link
```
...
link target is recorded in `.ethos-data-materialized.json` inside the directory, so a
...
| Symptom | Command |
|---|---|
| "is my data still right?" | `ethos-data verify --all` |
| "is my data *really* still right?" | `ethos-data verify --all --deep` |
| "something drifted, fix it" | `ethos-data verify --all --deep --repair` |
| "this link is about to break" | `ethos-data materialize <dataset>` |
| "where is this even coming from?" | `ethos-data config show` |
