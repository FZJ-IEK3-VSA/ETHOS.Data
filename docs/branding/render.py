    ...
    with tempfile.TemporaryDirectory(prefix="ethos-logo-") as directory:
        ...
        if not icon:
            font_info = subprocess.check_output(
                [need("pdffonts"), str(temporary / "logo.pdf")],
                text=True,
            )
            if "WeissenhofGrotesk-Bold" not in font_info:
                raise SystemExit(
                    f"{name}: expected Weissenhof Grotesk Bold, got:\n{font_info}"
                )
        rendered = temporary / "logo.svg"
        subprocess.run(
            [need("pdftocairo"), "-svg", str(temporary / "logo.pdf"), str(rendered)],
            check=True,
        )
...
def embedded(asset: Path, prefix: str, x: float, y: float, scale: float) -> ET.Element:
    ...
    for element in logo.iter():
        for key, value in list(element.attrib.items()):
            if key == "id":
                ...
                element.set(
                    key, " ".join(ids.get(part, part) for part in value.split())
                )
    ...
    logo.attrib.update(
        x=str(x), y=str(y), width=str(width * scale), height=str(height * scale)
    )
    return logo
...
def background(
    root: ET.Element, width: int, height: int, colour: str, y: int = 0
) -> None:
    ET.SubElement(
        root,
        f"{{{SVG}}}rect",
...
def preview_pair(package: dict) -> None:
    ...
    for index, (theme, colour) in enumerate(
        (("light", "#FFFFFF"), ("dark", "#1E2129"))
    ):
        background(root, 1200, 260, colour, y=index * 260)
        asset = OUT / f"{package['slug']}-logo-{theme}.svg"
        _, _, width, height = map(
            float, ET.parse(asset).getroot().attrib["viewBox"].split()
        )
...
def family_preview(packages: list[dict], dark: bool) -> None:
    ...
    widths = [
        float(ET.parse(asset).getroot().attrib["viewBox"].split()[2])
        for asset in assets
    ]
...
def favicon_preview(packages: list[dict]) -> None:
    root = canvas(
        760, 64 + len(packages) * 104, "ETHOS favicons at 16, 32, 48 and 64 pixels"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--all", action="store_true", help="render all entries in packages.json"
    )
    parser.add_argument(
        "--preview",
        action="store_true",
        help="also export PNG favicons and review sheets",
    )
    parser.add_argument(
        "--font-dir",
        type=Path,
        default=DEFAULT_FONTS,
        help="local Weissenhof Grotesk OTF directory",
    )
    ...
    for style in FONT_STYLES:
        if not (fonts / f"WeissenhofGrotesk-{style}.otf").is_file():
            raise SystemExit(
                f"Missing Weissenhof Grotesk {style} in {fonts}; pass --font-dir."
            )
