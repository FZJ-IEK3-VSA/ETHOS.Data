...
<figure markdown="span">
  ![Catalogue maintenance decomposition: Manifest builder reads source files and writes metadata; Uploader transfers listed files through rclone and verifies through HTTP; Public-view generator filters metadata; Namespace builder creates shared-cache links.](../../assets/diagrams/architecture-maintainer-light.svg#only-light){ .diagram }
  ![Catalogue maintenance decomposition: Manifest builder reads source files and writes metadata; Uploader transfers listed files through rclone and verifies through HTTP; Public-view generator filters metadata; Namespace builder creates shared-cache links.](../../assets/diagrams/architecture-maintainer-dark.svg#only-dark){ .diagram }
</figure>

Building metadata, uploading bytes, generating a public view, and linking existing
storage have different failure modes and authority requirements. They remain
separate commands so maintainers can validate and review each output. Their shared
...
| Namespace builder (`maintain.namespace`) | Plan and apply shared-cache symbolic links to existing local source trees | Source YAML and filesystem paths → namespace plan/links |
