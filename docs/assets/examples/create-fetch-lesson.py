...
def create_lesson(destination: Path) -> None:
    ...
    package = {
        ...
        "resources": [{
            "name": "temperatures", "path": "temperatures.csv", "bytes": len(data),
    ...
    index = {
        ...
        "datasets": [{
            "name": dataset, "path": "datapackage.json", "ethos:access": "public",
            "ethos:remote_prefix": dataset, "ethos:license_status": "resolved",
        }],
    }
    for name, content in (("datapackage.json", package), ("datacatalog.json", index)):
        (catalogue / name).write_text(json.dumps(content, indent=2) + "\n", encoding="utf-8")
    ...
    restricted_package = {
        "name": "lesson-licensed", "title": "Synthetic access-control example",
        "ethos:access": "restricted", "ethos:visibility": "hidden",
        "ethos:license_status": "resolved",
        "ethos:restriction": "Practice data: use the lesson's restricted-installation directory.",
        "licenses": [{"name": "CC0-1.0"}],
        "resources": [{
            "name": "factor", "path": "factor.csv", "bytes": len(restricted_bytes),
            "hash": "sha256:" + hashlib.sha256(restricted_bytes).hexdigest(),
        }],
    }
    (internal / "restricted.json").write_text(json.dumps(restricted_package), encoding="utf-8")
    internal_index = {**index, "datasets": [*index["datasets"], {
        "name": "lesson-licensed", "path": "restricted.json",
        "ethos:access": "restricted", "ethos:license_status": "resolved",
    }]}
    (internal / "datacatalog.json").write_text(json.dumps(internal_index), encoding="utf-8")
