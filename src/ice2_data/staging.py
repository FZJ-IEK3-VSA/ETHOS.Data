...
entries are dataset names, exactly like the public cache:
    ...
    ice2-data staging add my-new-dataset /scratch/me/new-data
...
staged data can be verified, and :mod:`ice2_data.access` warns every time a job
reads it.
...
launder data into a result:
  ...
  * **Staged data is never uploaded.** These datasets exist only on this
    machine; ``ice2-data catalog`` never sees them, because the staging root is a
...
def _require_root(explicit: str | Path | None = None) -> Path:
    ...
    if root is None:
        raise SystemExit(
            ...
            "    ice2-data config set-staging-cache /path/to/ice2_data_staging"
        )
    return root
...
def add(
    ...
    if entry.exists() or entry.is_symlink():
        raise SystemExit(
            ...
            f"Remove it first:  ice2-data staging remove {name}"
        )
    ...
    if entry.is_symlink():
        ...
        if not force:
            raise SystemExit(
                ...
                f"If that is what you mean:  ice2-data staging remove {name} --force"
            )
        import shutil
...
def synthesize(name: str, directory: Path, access: str = STAGING_ACCESS) -> Dataset:
    ...
    entry = {
        ...
        "ice2:file_count": len(resources),
        "ice2:total_bytes": total,
    }
