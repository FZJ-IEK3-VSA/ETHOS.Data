...
the old link target is recorded in ``.ice2-materialized.json`` so that a copy
...
PROVENANCE_FILE = ".ice2-materialized.json"
    ...
    for name in sorted(set(names)):
        try:
            ...
            reports.append(MaterializeReport(
                ...
                "not describe it; `ice2-data catalog link-cache --prune` removes stale links"))
            continue
