...
#: ``ice2-data verify`` can say something more useful than "not found".
...
def _restricted_location(
    ...
    lines.append("    ice2-data config set-restricted-cache /path/to/ice2_data_restricted "
                 "--scope environment")
    lines.append(f"    ice2-data config set-root {dataset.name} /path/to/{dataset.name} "
                 "--scope environment    # just this one")
    lines.append("")
    lines.append("If you do not -- working away from the institute cluster, say -- then "
                 "carry on without it:")
    lines.append("    ice2-data ... --skip-unavailable")
    lines.append("    ice2-data config set-skip-unavailable true    # once, for this machine")
...
def locate(
    ...
    ``roots`` accepts a :class:`~ice2_data.config.Roots`, or a bare path meaning
    ...
    for resource in resources:
        ...
        if not catalog.publication_url:
            raise AccessError(
                ...
                f"from.\n"
                f"Either set ice2:publication_url in catalog.yaml, or, while the data is "
                f"not yet uploaded, point at the copy on this machine:\n"
                f"    ln -s /path/to/{dataset.name} {entry}\n"
                f"or, for this one dataset only:\n"
                f"    ice2-data config set-root {dataset.name} /path/to/{dataset.name} "
                f"--scope environment"
            )
