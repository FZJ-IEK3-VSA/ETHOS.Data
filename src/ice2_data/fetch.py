...
the same file land on the same file. There is nothing to synchronise and no
link farm to maintain -- the second tool simply finds the file already there.
"""
...
from .access import AccessError, Location, check_missing, locate
from .catalog import Catalog, Resource
from .config import ENV_VAR, dataset_roots, resolve_cache_dir

__all__ = ["AccessError", "DataFiles", "ENV_VAR", "cache_dir", "download", "local_path", "plan"]
...
def cache_dir(explicit: str | Path | None = None) -> Path:
    ...
    """
    return resolve_cache_dir(explicit).value
...
def plan(catalog: Catalog, resources: list[Resource], root: Path | None = None) -> dict:
    """Report what a fetch would do, without touching the network."""
    root = root or cache_dir()
    locations = locate(catalog, resources, root, dataset_roots())
...
def download(
    ...
    Resources with a configured local root are used in place and never copied;
    the rest are downloaded into the shared cache, skipping anything already
    ...
    unreadable = check_missing(locations)
    if unreadable:
        listing = "\n".join(f"    {loc.path}" for loc in unreadable[:8])
        more = "" if len(unreadable) <= 8 else f"\n    ... and {len(unreadable) - 8} more"
        raise AccessError(
            f"{len(unreadable)} file(s) are missing from their configured local root:\n"
            f"{listing}{more}\n"
            "Check the dataset_roots entry with `ice2-data config show`."
        )
    ...
    # Return in catalogue order, not download order.
